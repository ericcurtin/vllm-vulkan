...
class VulkanAttentionBackendImpl(CPUAttentionBackendImpl):
    ...
    def do_kv_cache_update(
        ...
        """Write this step's new K/V tokens, to vLLM's CPU cache and to the
        Vulkan mirror.

        vLLM 0.29 split the KV-cache write out of `forward` into its own
        `unified_kv_cache_update` custom op (vllm/model_executor/layers/
        attention/attention.py), ordered against `unified_attention` by a
        torch.compile data dependency rather than by being the same call.
        So the write lives here and the read lives in `forward`, and the
        two can no longer share one `ctx.execute_batch` submit, which is
        what the merged write+decode path removed alongside this used to do.
        """
    ...
    def forward(
        ...
        if num_actual_tokens > 0 and self._try_vulkan_decode(
            query=query,
            key=key,
            value=value,
            kv_cache=kv_cache,
    ...
    def _supports_vulkan_decode(
        ...
        *,
        ...
        if self.alibi_slopes is not None or bool(self.logits_soft_cap):
            return False
        # vLLM 0.29 stores "no sliding window" as the int -1
        # (CPUAttentionBackendImpl.__init__); 0.20 stored the tuple (-1, -1).
        if self.sinks is not None or self.sliding_window != -1:
            return False
...
def _parse_kv_cache_shape(shape: tuple[int, ...]) -> tuple[int, int, int, int]:
    """(num_blocks, num_kv_heads, block_size, head_size) from vLLM's HND
    KV cache shape ``(num_blocks, num_kv_heads, block_size, 2 * head_size)``.
    ...
    view of vLLM's buffer, so these four numbers are all that vLLM's own
    layout contributes -- reading `head_size` out of the doubled last
    dimension is the whole of it.
    """
    if len(shape) == 4 and shape[3] % 2 == 0:
        return shape[0], shape[1], shape[2], shape[3] // 2
    raise ValueError(
        f"expected KV cache shape [blocks, heads, block, 2*dim], got {shape}"
    )


def _kv_cache_storage_key(kv_cache: torch.Tensor) -> int:
    """Identity of one layer's KV cache, as the address its own data starts at.

    `.data_ptr()`, NOT `.untyped_storage().data_ptr()`: vLLM allocates one
    buffer per `KVCacheTensor` and hands every layer sharing it a view at its
    own offset, so the storage base is identical across layers. Measured on
    vLLM 0.29 with Qwen3-0.6B: 28 layers, 28 distinct `.data_ptr()` values,
    1 distinct storage base. Keying on the storage base collapsed all 28
    layers onto the first one's Vulkan mirror -- every layer then decoded
    against another layer's K/V, and generation came out as word salad while
    each kernel remained individually correct.
    """
    return int(kv_cache.data_ptr())
