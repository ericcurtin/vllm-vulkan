...
class VulkanPlatform(Platform):
    ...
    def get_device_total_memory(cls, device_id: int = 0) -> int:
        ...
        Since we use CPUWorker and KV cache is allocated on CPU,
        we return CPU memory instead of Vulkan GPU memory.
        """
    ...
    def check_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        # Set CPU KV cache space from environment variable (required for CPU worker)
        # This tells vLLM how much memory to allocate for KV cache on CPU
        if cache_config.cpu_kvcache_space_bytes is None:
            kv_space_gb = float(os.environ.get("VLLM_CPU_KVCACHE_SPACE", "4"))
            cache_config.cpu_kvcache_space_bytes = int(kv_space_gb * 1024**3)

        # Disable cascade attention (not supported)
        if model_config is not None:
            model_config.disable_cascade_attn = True

        # Log memory configuration (CPU memory for KV cache)
        total_mem = cls.get_device_total_memory()
        available_mem = cls.get_device_available_memory()
        logger.info(
            f"CPU memory for KV cache: {total_mem / 1e9:.1f}GB total, "
    ...
    def get_attn_backend_cls(
        ...
        return str(AttentionBackendEnum.CPU_ATTN.get_path())
