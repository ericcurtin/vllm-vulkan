...
def set_context(ctx: "VulkanContext") -> None:
    ...
    try:
        ...
        if devs:
            ...
            if dtype == "cpu" or any(s in name for s in
                                     ("llvmpipe", "lavapipe", "swiftshader", "software")):
                logger.info(
                    "Vulkan device '%s' is a software renderer; "
                    "GPU dispatch disabled.", devs[0].get("name", ""))
                _ctx = None
    except Exception:
        pass
...
def _matvec_pc(T: int, K: int, N: int) -> bytes:
    """Pack push constants for mul_mat_vec_f32_f32_f32_subgroup."""
    return struct.pack(
        "13I",
        K, K, K, N,        # ncols, stride_a, stride_b, stride_d
        K * N, K, N,       # batch_stride_a, batch_stride_b, batch_stride_d
        0,                 # fusion_flags
        0,                 # base_work_group_y
        1, T, T, 1,        # ne02, ne12, broadcast2, broadcast3
    )
...
def rms_norm(
    ...
    from vllm_vulkan import batch_ctx as _bc  # noqa: PLC0415
    ctx = get_context()
...
def _cpu_rms_norm(x: torch.Tensor, weight: "torch.Tensor | None", eps: float) -> torch.Tensor:
    xf = x.float()
...
def linear(
    ...
    if T < _MATVEC_THRESHOLD and "mul_mat_vec_f32_f32_f32_subgroup" in ctx.available_shaders():
...
def rms_norm_then_linear(
    ...
    w_lin_binding  = w_lin_gpu  if w_lin_gpu  is not None else _to_bytes(linear_weight.float())
